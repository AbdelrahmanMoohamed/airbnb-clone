...
namespace BLL.Services.Impelementation
{
    public class FaceIdService : IFaceIdService
    {
        ...
        public FaceIdService(UserManager<User> userManager, IFaceIdRepo faceRepo, FaceRecognition faceRecognition, IMapper mapper, ITokenService tokenService)
        {
        ...
        // Register a new face
        public async Task<Response<FaceId>> RegisterFaceAsync(RegisterFaceIdVM model)
        {
            try
            {
                ...
                {
                    return new Response<FaceId>(null, "No face detected in the image", false);
                }
                ...
                if (isAdded)
                {
                    return new Response<FaceId>(faceId, "Face registered successfully", true);
                }
                return new Response<FaceId>(null, "Failed to register face", false);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<Response<bool>> UpdateFaceAsync(UpdateFaceIdVM model)
        {
            try
            {
                // 1) Extract encoding from uploaded image file
                ...
                var encoding = await ExtractEncoding(model.imageFile);
                if (encoding == null)
                    return new Response<bool>(false, "No face detected in the image", false);
                ...
                if (isUpdated)
                {
                    return new Response<bool>(true, "Face updated successfully", true);
                }
                return new Response<bool>(false, "Failed to update face", false);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<Response<IEnumerable<FaceId>>> VerifyFaceByUserIdAsync(Guid userId)
        {
            try
            {
                ...
                if (!faces.Any())
                    return new Response<IEnumerable<FaceId>>(Enumerable.Empty<FaceId>(), "No faces found for this user", false);

                return new Response<IEnumerable<FaceId>>(faces, "Faces retrieved successfully", true);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<Response<string>> VerifyFaceLoginAsync(IFormFile image)
        {
            try
            {
                ...
                if (encoding == null)
                    return new Response<string>(null, "No face detected", false);
                ...
                if (users == null || !users.Any())
                    return new Response<string>(null, "No users with faces in database", false);
                ...
                if (bestUser != null && bestDistance < threshold)
                {
                    ...
                    // determine role
                    var roles = await _userManager.GetRolesAsync(bestUser);
                    var role = roles.FirstOrDefault() ?? "Guest";
                    var token = _tokenService.GenerateToken(bestUser.Id, role, bestUser.FullName);
                    return new Response<string>(token, $"Login success (distance = {bestDistance:F4})", true);
                }

                // No good match found
                return new Response<string>(null, $"Face not recognized (best distance = {bestDistance:F4})", false);
            }
            catch
            {
                throw;
        ...
        public async Task<Response<bool>> DeleteFaceAsync(Guid userId)
        {
            try
            {
                // 1) Validate userId
                ...
                if (string.IsNullOrEmpty(userId.ToString()))
                    return new Response<bool>(false, "User not authenticated", false);
                ...
                if (deleted)
                {
                    return new Response<bool>(true, "Face deleted successfully", true);
                }
                return new Response<bool>(false, "FaceId not found", false);
            }
            catch
            {
                throw;
